...
import {
  ...
  GoToSignUp, FetchChannels, FetchUnreadRooms, SetToken
} from '../../store/actions'
...
export default class Login extends Component {
  ...
  login = async () => {
    ...
    if (this.state.username.length != 11 || this.state.password.length < 5) {
      Alert.alert(
        I18n.t('tips'),
        I18n.t('Login.fillInfo'),
        [
          { text: 'OK', onPress: () => { } },
        ]
      )
      ...
      const res = await api.userLogin(this.state)
      switch (res.status) {
        case 200: {
          // const token = res.json()
          // await this.prop.dispatch(SetToken(token))
